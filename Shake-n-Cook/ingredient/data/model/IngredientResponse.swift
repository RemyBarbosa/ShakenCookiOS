...
struct IngredientAPI: Decodable, Hashable {
    ...
    func toIngredientFirebase() -> IngredientFirebase? {
        ...
        return IngredientFirebase(
            ...
            pictureUrl: image,
            searchArray: knownAs!.split(separator: " ").map(String.init)
        )
    }
}
...
struct IngredientFirebase: Codable, Hashable {
    ...
    static func defaultIngredient() -> IngredientFirebase {
        return IngredientFirebase(id: "id", name: "name", nameFr: "nameFr", label: "label", nutrient: NutrientFirebase.defaultNutrient(), pictureUrl: nil, searchArray: [String]())
    }
    
    func toUnselectedIngredient() -> Ingredient {
        return Ingredient(ingredientFirebase: self, isSelected: false)
    }
}
