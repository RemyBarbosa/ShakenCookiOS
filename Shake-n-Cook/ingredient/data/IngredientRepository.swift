...
class IngredientRepository {
    ...
    func getIngredients(query:String, completion: @escaping (Result<[IngredientAPI], Error>) -> Void) {
        ...
        let firebaseQuery = firestoreIngredientCollection
            .whereField("searchArray", arrayContainsAny: queryArray)
    ...
    func saveShakeIngredientOnFirebase(ingredient: IngredientFirebase, image: UIImage, completion: @escaping (Result<IngredientFirebase, Error>) -> Void) {
        ...
        guard let imageData = image.jpegData(compressionQuality: 0) else {
            completion(.failure(ShakeError.genericError(message: "Error creating JPEG data from image")))
            return
        }
