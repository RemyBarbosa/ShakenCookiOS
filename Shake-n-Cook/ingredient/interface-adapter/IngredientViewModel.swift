...
class IngredientViewModel: ObservableObject {
    ...
    @Published var lastBarcodeValue: String?
    ...
    func getIngredient() {
        ...
        ingredientRepository.fetchIngredients(withIds: [barcodeValue]) { [weak self] response in
            ...
            case .success(let ingredients):
                ...
                self?.selectedIngredients.append(Ingredient(ingredientFirebase: ingredient, isSelected: true))
            case .failure( _):
                // todo handle error with state
                self?.state = IngredientState.error
            }
            guard let defaultBarcode = self?.barcodeDefaultValue else {
                return
            }
            self?.lastBarcodeValue = self?.barcodeValue
            self?.barcodeValue = defaultBarcode
        }
    }
}
